...
int BPF_PROG(tcp_rcv, struct sock *sk)
{
	...
	u64 key, slot;
	u32 srtt;
	bpf_printk("sport: %u, dport: %u\n", BPF_CORE_READ(inet, inet_sport), BPF_CORE_READ(sk, __sk_common.skc_dport));
	bpf_printk("saddr: %u, daddr: %u\n", BPF_CORE_READ(inet, inet_saddr), BPF_CORE_READ(sk, __sk_common.skc_daddr));
