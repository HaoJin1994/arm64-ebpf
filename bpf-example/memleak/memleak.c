...
#ifndef USE_BLAZESYM_API
static void print_stack_frames_by_blazesym();
#else
static void print_stack_frames_by_blazesym_api();
#endif
...
static void resolve_libc_path(char *buf, size_t size)
{
	Dl_info info;

	/* 通过已链接的 printf 符号获取 libc 完整路径 */
	if (dladdr((void *)printf, &info) && info.dli_fname && info.dli_fname[0] == '/') {
	...
	if (handle) {
		if (dladdr(dlsym(handle, "printf"), &info) && info.dli_fname && info.dli_fname[0] == '/') {
			strncpy(buf, info.dli_fname, size - 1);
			buf[size - 1] = '\0';
			dlclose(handle);
			return;
	...
	/* 兜底：保留短名称 */
	strncpy(buf, default_object, size - 1);
	buf[size - 1] = '\0';
}
...
int main(int argc, char *argv[])
{
	...
	if (!strlen(env.object)) {
		...
		resolve_libc_path(env.object, sizeof(env.object));
		printf("using default object: %s\n", env.object);
	}
...
#ifdef USE_BLAZESYM_API
	print_stack_frames_func = print_stack_frames_by_blazesym_api;
#else
...
#else
void print_stack_frames_by_blazesym_api()
{
    int cnt;
	sym_info_t *results = malloc(env.perf_max_stack_depth * sizeof(sym_info_t));
	if (!results) {
		fprintf(stderr, "Failed to allocate memory for results\n");
		return;
	}
	for (int i = 0; i < env.perf_max_stack_depth && stack[i] != 0; i++)
		stack[i]--;
	if (env.kernel_trace) {
		cnt = resolve_kernel_symbols(stack, env.perf_max_stack_depth, results);
	}
	else{
		cnt = resolve_symbols(env.pid, NULL, 0, stack, env.perf_max_stack_depth, results);
	}	
	if (cnt < 0) {
	...
	for(int i = 0; i < cnt; i++){
		if(results[i].addr == 0)
			break;
		if (!results[i].name || !results[i].module)
			printf("\t%zu [<%016lx>] <%s>\n", i, results[i].addr, results[i].name ? results[i].name : "null sym");
		else if (results[i].src_path)
			printf("\t%zu [<%016lx>] %s+0x%zx %s:%u\n", i, results[i].addr, results[i].name, results[i].module_offset, results[i].src_path, results[i].src_line);
		else
			printf("\t%zu [<%016lx>] %s+0x%zx\n", i, results[i].addr, results[i].name, results[i].module_offset);
	}
	free(results);
}
#endif
