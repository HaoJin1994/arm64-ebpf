...
#include "profile_stack.skel.h"
#include "profile_stack.h"
...
static void show_stack_trace(__u64 *stack, int stack_sz, pid_t pid,
							 const char *comm, __u64 load_base)
{
#if 0
	...
	if (pid == 0)
	{
		int kstack_depth = stack_sz;
		if (kstack_depth > 0)
		{
			sym_info_t kframes[kstack_depth];
			int kn = resolve_kernel_symbols(stack, kstack_depth, kframes);
			printf("[Kernel Stack]\n");
			for (int i = 0; i < kn; i++)
			{
	...
	else
	{
		...
		sym_info_t frames[stack_sz];
		int n = resolve_symbols(pid, comm, load_base,
								stack, stack_sz, frames);
		if (n <= 0)
		{
...
int main(int argc, char *const argv[])
{
	...
	struct profile_stack_bpf *skel = NULL;
	...
	skel = profile_stack_bpf__open_and_load();
	if (!skel)
	{
...
cleanup:
	...
	profile_stack_bpf__destroy(skel);
	free(online_mask);
	return -err;
}
