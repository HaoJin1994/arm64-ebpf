...
void print_log2_hist(unsigned int *vals, int vals_size, const char *val_type)
{
	...
	if (idx_max <= 32)
		stars = stars_max;//idx_max <= 32 时，星号宽度为 stars_max
	else
		stars = stars_max / 2;//idx_max > 32 时，星号宽度为 stars_max / 2

	for (i = 0; i <= idx_max; i++)
	{
		low = (1ULL << (i + 1)) >> 1; //区间[low,high]的下限
		high = (1ULL << (i + 1)) - 1;///区间[low,high]的上限
		if (low == high)
			low -= 1;//单点区间，下限减1
		val = vals[i];
		width = idx_max <= 32 ? 10 : 20;//idx_max <= 32 时，宽度为 10，否则为 20
