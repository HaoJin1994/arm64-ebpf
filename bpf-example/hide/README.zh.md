...
linux_dirent64 的存储方式：紧密拼接的变长数组,结构体定义如下：
```c
struct linux_dirent64 {
    __u64   d_ino;       // 8 bytes — inode 号
    __s64   d_off;       // 8 bytes — 到下一个结构的偏移（getdents64 内部使用）
    __u16   d_reclen;    // 2 bytes — 本结构的实际总长度（可变！）
    __u8    d_type;      // 1 byte  — DT_DIR(4), DT_REG(8) 等
    char    d_name[];    // 柔性数组 — 文件名，以 '\0' 结尾
};
```
内存布局如下：
```
