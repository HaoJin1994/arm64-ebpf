...
static int parse_args(int argc, char **argv)
{
	...
	while ((option = getopt_long(argc, argv, "d:p:s:vh", options, NULL)) != -1) {
		switch (option) {
		case 'd':
...
static int run_iterator(struct bpf_program *program)
{
	...
	while ((length = read(iter_fd, buffer, sizeof(buffer) - 1)) > 0) {
		buffer[length] = '\0';
		printf("%s", buffer);
	}
	if (length < 0) {
...
int main(int argc, char **argv)
{
	...
	skel->rodata->target_state = env.state;

	err = tcp_status_bpf__load(skel);
	if (err) {
		fprintf(stderr,
			"failed to load TCP status program: %s\n"
	...
	err = run_iterator(skel->progs.tcp_link_status);
	if (err)
		goto cleanup;


cleanup:
	tcp_status_bpf__destroy(skel);
	return err != 0;
}
